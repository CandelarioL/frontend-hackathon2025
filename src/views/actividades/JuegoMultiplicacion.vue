<template>
  ...
  >
    ...
    >
      <img
        :class="[
          'w-52 sm:w-64 drop-shadow-2xl transition-transform duration-700',
          saltando ? 'jump' : ''
        ]"
    ...
    >
      ...
      <h2 class="text-2xl font-semibold text-gray-800 mb-8">
        Multiplica los números y observa las pistas 👀
      </h2>
      ...
      <div class="flex justify-center flex-wrap gap-3 mb-8">
        <div v-for="n in limiteIconos" :key="n" class="text-4xl">
          {{ iconoAleatorio() }}
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      numero2: Math.floor(Math.random() * 5) + 2,
      respuesta: "",
      mensaje: "",
      esCorrecto: false,
      contador: 0,
      puntuacion: 0,
      mensajeMascota: "💬 ¡Intenta con calma, tú puedes lograrlo! 🐶💪",
      iconos: ["🍎", "🎈", "⭐", "🧩", "🍬", "⚽", "🎲", "🦋", "🍉", "🏓"],
      saltando: false,
    };
  },
  computed: {
    ...
    // Limita la cantidad de íconos para evitar saturar la pantalla
    limiteIconos() {
      return this.resultado > 20 ? 20 : this.resultado;
    },
  },
  methods: {
    generarNumero() {
      return Math.floor(Math.random() * 5) + 2; // Devuelve un número entre 2–6
    },
    iconoAleatorio() {
    ...
    verificar() {
      ...
      if (parseInt(this.respuesta) === correcta) {
        ...
        this.mensajeMascota =
          "🎉 ¡Excelente! ¡Multiplicaste perfecto, eres increíble! 🐾💫";
        this.esCorrecto = true;
        this.contador++;
        this.puntuacion += 15; // Puntos por acierto
        this.saltarMascota();
      } else {
        this.mensaje = `❌ Incorrecto, era ${correcta}`;
        this.mensajeMascota =
          "😿 No pasa nada, inténtalo otra vez con calma 💪";
      ...
      setTimeout(() => {
        ...
        this.mensaje = "";
        this.mensajeMascota =
          "💬 ¡Intenta con calma, tú puedes lograrlo! 🐶💪";
      }, 2200);
    },
    saltarMascota() {
      this.saltando = true;
    ...
    leerInstruccion() {
      const texto = `Atención: Multiplica ${this.numero1} por ${this.numero2} y escribe el resultado.`;
      const speech = new SpeechSynthesisUtterance(texto);
      speech.lang = "es-ES";
      speech.rate = 0.85;
      speech.pitch = 1.2;
      speech.volume = 1.0; // más fuerte pero sin distorsionar
